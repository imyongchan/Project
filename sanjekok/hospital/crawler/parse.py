...
def parse_hospitals(raw_json: dict) -> List[Dict]:
    ...
    - h_address       : rn_adres / RN_ADRES 우선 사용, 없으면 adres / ADRES 사용
    - 이름이나 주소 둘 중 하나라도 없으면 해당 레코드는 버린다.
    - h_rc, h_rc_info 는 사용하지 않으므로 항상 빈 문자열로 둔다.
    """
    ...
    for it in items:
        ...
        # -------- 주소: rn_adres 우선, 없으면 adres --------
        rn_addr = (
            it.get("rn_adres")
            or it.get("RN_ADRES")
            or ""
        )
        rn_addr = rn_addr.strip() if isinstance(rn_addr, str) else ""
        ...
        jb_addr = jb_addr.strip() if isinstance(jb_addr, str) else ""

        # rn_adres 우선, 없으면 adres 사용
        address = rn_addr or jb_addr
